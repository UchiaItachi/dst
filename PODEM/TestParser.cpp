...
const std::regex TestParser::TEST_INPUT_REGEX("\\s*Net\\s+(\\d+)\\s+s-a-(0|1)\\s*");
...
void TestParser::parseTestInputFile(CircuitInfo& ci)
{
  ...
  for (std::string line; std::getline(file, line);)
  {
    ...
    {
      TestVector tv{matches[1],matches[2]};
...
FaultList_t TestParser::getFaultSpace(const std::string& flFile)
{
  FaultList_t restrictedFaultList;

  std::ifstream file;
  file.open(flFile, std::fstream::in);
  if (!file.is_open())
  {
    std::cout << "\nError: Couldn't open the Fault list file : " << flFile << std::endl;
    throw std::exception();
    return FaultList_t();
  }
  ...
  while (std::getline(file, line))
  {
    ...
    if (iss >> token)
    {
      uint32_t node = static_cast<uint32_t>(std::strtoul(token.c_str(), nullptr, 10));
      StuckAtFaults_t &fl = restrictedFaultList[node];
      if (iss >> token)
      {
        uint32_t stuckAtValue = static_cast<uint32_t>(std::strtoul(token.c_str(), nullptr, 10));
        fl.push_back(stuckAtValue);
      }
      if (iss >> token)
      {
        uint32_t stuckAtValue = static_cast<uint32_t>(std::strtoul(token.c_str(), nullptr, 10));
  ...
  return restrictedFaultList;
}

void TestParser::printCustomSummary(TestVectors_t& vectors)
{
  ...
  for (auto& circuit: circuitInfo_)
  {
    ...
    output << std::endl;
    for (auto& testVector: vectors)
    {
      //bool pass = testVector.output == testVector.expectedOutput;
      output //<< "\t" << "Result : " << (pass ? "PASS" : "FAIL")
        << "Input : " << testVector.inputNet << " "
        << "\t" << "Good Circuit Output : " << testVector.output << " "
        << "\n" << "Detected Fault List : \n" << testVector.faultOutput << " "
